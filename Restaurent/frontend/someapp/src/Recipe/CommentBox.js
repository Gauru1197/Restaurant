...
function CommentBox() {
  ...
  return (
    <div className='commentBox'>
       <p className='p1 m-3' style={{fontSize:'30px'}} data-aos="fade-right">Add a Review <span style={{textTransform:'capitalize'}}>{JSON.parse(auth).name} </span></p>
...
{/* comment */}
  ...
  <textarea className="form-control" id="exampleFormControlTextarea1" rows="5" placeholder='Add a Review' value={comment} onChange={(e)=>setComment(e.target.value)}></textarea>
  {error && !comment && <span id="emailHelp" className="form-text" style={{color:'red',fontSize:"15px"}}>Please enter rreview</span>}
</div>
